...
with napari.gui_qt():
    ...
    def load_source_images():
        ...
        for imPath in sorted(imgFileList):
            ...
            images.append(
                numpy.memmap(raw_fn, mode="r", shape=(imageHeight, imageWidth, 3))
            )
    ...
    def load_grayscale_images():
        ...
        for imPath in sorted(imgFileList):
            ...
            grayscales.append(
                numpy.memmap(grayscale_fn, mode="r", shape=(imageHeight, imageWidth))
            )
    ...
    def load_aligned_images():
        ...
        for imPath in sorted(imgFileList):
            ...
            if os.path.isfile(_raw_aligned):
                grayscales.append(
                    numpy.memmap(
                        _raw_aligned, mode="r", shape=(imageHeight, imageWidth, 3)
                    )
                )
            else:
                grayscales.append(
                    numpy.memmap(_raw_fn, mode="r", shape=(imageHeight, imageWidth, 3))
                )
    ...
    def load_laplacian_gradients():
        ...
        for imPath in sorted(imgFileList):
            ...
            laplacians.append(
                numpy.memmap(_laplacian_fn, mode="r", shape=(imageHeight, imageWidth))
            )
