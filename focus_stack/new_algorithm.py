...
memmap_extensions = [
    rgb_memmap_extension,
    grayscale_memmap_extension,
    laplacian_memmap_extension,
]
...
def load_images():
    ...
    def load_single_image(image_path):
        ...
        memmapped_rgb = np.memmap(
            image_path + rgb_memmap_extension, mode="w+", shape=SHAPE
        )
        memmapped_rgb[:] = image_rgb
        del memmapped_rgb

        memmapped_grayscale = np.memmap(
            image_path + grayscale_memmap_extension,
            mode="w+",
...
def align_images(image_paths):
    ...
    def align_single_image(im1_path, im2_path):
        ...
        # Get memmap's
        im1_gray = np.memmap(
            im1_path + grayscale_memmap_extension, mode="r", shape=(SHAPE[0], SHAPE[1])
        )
        im2_gray = np.memmap(
            im2_path + grayscale_memmap_extension, mode="r", shape=(SHAPE[0], SHAPE[1])
        )
...
def focus_stack(image_paths):
    ...
    def load_images():
        for im_path in image_paths:
            global SHAPE
            images.append(
                np.memmap(im_path + rgb_memmap_extension, mode="r", shape=SHAPE)
            )
            laplacians.append(
                np.memmap(
                    im_path + laplacian_memmap_extension,
                    mode="r",
                    shape=(SHAPE[0], SHAPE[1]),
                    dtype="float64",
                )
    ...
    def calculate_pixels():
        for y in range(0, images[0].shape[0]):  # Loop through vertical pixels (columns)
            for x in range(
                0, images[0].shape[1]
            ):  # Loop through horizontal pixels (rows)
                yxlaps = abs(
                    laplacians[:, y, x]
                )  # Absolute value of laplacian at this pixel
...
dask.compute(
    dask.delayed(calculate_edges_laplacian)(image_paths)
)  # Calculate edges with a laplacian gradient
